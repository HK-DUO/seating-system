...
function SeatMain() {
  ...
  const onView = async (num: number): Promise<any> => {
    ...
    let res = (await window.electron.viewReadingRoom(
      num,
    )) as ResponseEntity<READING_ROOM_DTO>;
    return res;
  };
      ...
      <div>
        <div>
          ...
          <button
            ...
            className={roomNum == 2 ? 'bg-blue text-white' : 'bg-light-blue'}
          >
            열람실
          </button>
        </div>
        <RoomLayout
          type={roomData?.selectRoom}
          seatNum={seatNum}
