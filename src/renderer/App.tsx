...
function App() {
  ...
  const [askDbReset, setAskDbReset] = React.useState(localStorage.getItem("askDbReset"));
  ...
  const resetDb = async () => {
    await confirm("데이터초기화", "데이터를 초기화하시겠습니까?\n앱이 종료될때까지 다시 묻지 않습니다.").then((res)=>{
      if(res){
        window.electron.reset().then(async (res: any) => {
          if(res.code == 200){
            await alert("데이터초기화", "데이터가 성공적으로 초기화 되었습니다.").then(()=>{
