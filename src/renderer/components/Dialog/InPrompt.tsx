...
function InPrompt({ roomNum, seatNum, onClickOk, onClickCancel }: PropsType) {
  ...
  const [plusTime, setPlusTime] = useState<number>(2);

  useEffect(() => {
    window.electron.viewConfig().then((res) => {
      setPlusTime(res.data.reservation_time);
    });
  }, []);
