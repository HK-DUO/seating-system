...
function OutPrompt({seatNum, roomNum, onClickCancel, onClickOk}: PropsType) {
  ...
  return <div className="outPrompt">
    <h1>{"퇴실요청"}</h1>
    <div>
      <label>요청자이름</label><input onChange={onChange} value={value.name} name="name"/>
    </div>
    <div>
      <label>요청자전화번호</label><input onChange={onChange} value={value.number} onKeyDown={handleKeyDown} name="number"/>
    </div>
    <div>
    ...
    <div>
      <label>공석시간</label><input value={getExitTime(30)} disabled={true} />
    </div>
