...
import {createQuery} from '../db/query'
...
export function createTable() {
  const db = connect();
  db.exec(createQuery.createReservationTableQuery);
  db.exec(createQuery.createSeatTableQuery);
  db.exec(createQuery.createUserTableQuery);
  db.exec(createQuery.createReadingRoomTableQuery);
}
